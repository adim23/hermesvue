...
class CodesResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/codes";
     ...
     *
     * @see gr.adr.hermes.repository.search.CodesSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void createCodes() throws Exception {
        ...
        // Validate the Codes in Elasticsearch
        verify(mockCodesSearchRepository, times(1)).save(testCodes);
    }
    ...
    void createCodesWithExistingId() throws Exception {
        ...
        // Validate the Codes in Elasticsearch
        verify(mockCodesSearchRepository, times(0)).save(codes);
    }
    ...
    void putNewCodes() throws Exception {
        ...
        // Validate the Codes in Elasticsearch
        verify(mockCodesSearchRepository).save(testCodes);
    }
    ...
    void deleteCodes() throws Exception {
        ...
        // Validate the Codes in Elasticsearch
        verify(mockCodesSearchRepository, times(1)).deleteById(codes.getId());
    }

    @Test
    @Transactional
    void searchCodes() throws Exception {
        // Configure the mock search repository
        // Initialize the database
        codesRepository.saveAndFlush(codes);
        when(mockCodesSearchRepository.search("id:" + codes.getId(), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(codes), PageRequest.of(0, 1), 1));

        // Search the codes
        restCodesMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + codes.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(codes.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)));
