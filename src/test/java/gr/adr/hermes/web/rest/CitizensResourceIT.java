...
class CitizensResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/citizens";
     ...
     *
     * @see gr.adr.hermes.repository.search.CitizensSearchRepositoryMockConfiguration
     */
    @Autowired
    private CitizensSearchRepository mockCitizensSearchRepository;
    ...
    void createCitizens() throws Exception {
        ...
        // Validate the Citizens in Elasticsearch
        verify(mockCitizensSearchRepository, times(1)).save(testCitizens);
    }
    ...
    void createCitizensWithExistingId() throws Exception {
        ...
        // Validate the Citizens in Elasticsearch
        verify(mockCitizensSearchRepository, times(0)).save(citizens);
    }
    ...
    void putNewCitizens() throws Exception {
        ...
        // Validate the Citizens in Elasticsearch
        verify(mockCitizensSearchRepository).save(testCitizens);
    }
    ...
    void deleteCitizens() throws Exception {
        ...
        // Validate the Citizens in Elasticsearch
        verify(mockCitizensSearchRepository, times(1)).deleteById(citizens.getId());
    }

    @Test
    @Transactional
    void searchCitizens() throws Exception {
        // Configure the mock search repository
        // Initialize the database
        citizensRepository.saveAndFlush(citizens);
        when(mockCitizensSearchRepository.search("id:" + citizens.getId(), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(citizens), PageRequest.of(0, 1), 1));

        // Search the citizens
        restCitizensMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + citizens.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(citizens.getId().intValue())))
            .andExpect(jsonPath("$.[*].title").value(hasItem(DEFAULT_TITLE)))
