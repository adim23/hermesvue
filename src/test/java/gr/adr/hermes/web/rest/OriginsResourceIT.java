...
class OriginsResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/origins";
     ...
     *
     * @see gr.adr.hermes.repository.search.OriginsSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void createOrigins() throws Exception {
        ...
        // Validate the Origins in Elasticsearch
        verify(mockOriginsSearchRepository, times(1)).save(testOrigins);
    }
    ...
    void createOriginsWithExistingId() throws Exception {
        ...
        // Validate the Origins in Elasticsearch
        verify(mockOriginsSearchRepository, times(0)).save(origins);
    }
    ...
    void putNewOrigins() throws Exception {
        ...
        // Validate the Origins in Elasticsearch
        verify(mockOriginsSearchRepository).save(testOrigins);
    }
    ...
    void deleteOrigins() throws Exception {
        ...
        // Validate the Origins in Elasticsearch
        verify(mockOriginsSearchRepository, times(1)).deleteById(origins.getId());
    }

    @Test
    @Transactional
    void searchOrigins() throws Exception {
        // Configure the mock search repository
        // Initialize the database
        originsRepository.saveAndFlush(origins);
        when(mockOriginsSearchRepository.search("id:" + origins.getId(), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(origins), PageRequest.of(0, 1), 1));

        // Search the origins
        restOriginsMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + origins.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(origins.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)));
