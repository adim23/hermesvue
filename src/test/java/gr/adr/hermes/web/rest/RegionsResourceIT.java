...
class RegionsResourceIT {
     ...
     *
     * @see gr.adr.hermes.repository.search.RegionsSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void putNewRegions() throws Exception {
        ...
        // Validate the Regions in Elasticsearch
        verify(mockRegionsSearchRepository).save(testRegions);
    }
    ...
    void searchRegions() throws Exception {
        ...
        // Initialize the database
        regionsRepository.saveAndFlush(regions);
        when(mockRegionsSearchRepository.search("id:" + regions.getId(), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(regions), PageRequest.of(0, 1), 1));

        // Search the regions
        restRegionsMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + regions.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(regions.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)));
