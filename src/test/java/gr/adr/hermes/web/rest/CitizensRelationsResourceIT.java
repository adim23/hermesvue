...
class CitizensRelationsResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/citizens-relations";
    ...
    /**
     * This repository is mocked in the gr.adr.hermes.repository.search test package.
     *
     * @see gr.adr.hermes.repository.search.CitizensRelationsSearchRepositoryMockConfiguration
     */
    @Autowired
    private CitizensRelationsSearchRepository mockCitizensRelationsSearchRepository;
    ...
    void createCitizensRelations() throws Exception {
        ...
        // Validate the CitizensRelations in Elasticsearch
        verify(mockCitizensRelationsSearchRepository, times(1)).save(testCitizensRelations);
    }
    ...
    void createCitizensRelationsWithExistingId() throws Exception {
        ...
        // Validate the CitizensRelations in Elasticsearch
        verify(mockCitizensRelationsSearchRepository, times(0)).save(citizensRelations);
    }
    ...
    void putNewCitizensRelations() throws Exception {
        ...
        // Validate the CitizensRelations in Elasticsearch
        verify(mockCitizensRelationsSearchRepository).save(testCitizensRelations);
    }
    ...
    void deleteCitizensRelations() throws Exception {
        ...
        // Validate the CitizensRelations in Elasticsearch
        verify(mockCitizensRelationsSearchRepository, times(1)).deleteById(citizensRelations.getId());
    }

    @Test
    @Transactional
    void searchCitizensRelations() throws Exception {
        // Configure the mock search repository
        // Initialize the database
        citizensRelationsRepository.saveAndFlush(citizensRelations);
        when(mockCitizensRelationsSearchRepository.search("id:" + citizensRelations.getId(), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(citizensRelations), PageRequest.of(0, 1), 1));

        // Search the citizensRelations
        restCitizensRelationsMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + citizensRelations.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(citizensRelations.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)));
