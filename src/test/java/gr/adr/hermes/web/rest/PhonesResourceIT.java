...
class PhonesResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/phones";
     ...
     *
     * @see gr.adr.hermes.repository.search.PhonesSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void createPhones() throws Exception {
        ...
        // Validate the Phones in Elasticsearch
        verify(mockPhonesSearchRepository, times(1)).save(testPhones);
    }
    ...
    void createPhonesWithExistingId() throws Exception {
        ...
        // Validate the Phones in Elasticsearch
        verify(mockPhonesSearchRepository, times(0)).save(phones);
    }
    ...
    void putNewPhones() throws Exception {
        ...
        // Validate the Phones in Elasticsearch
        verify(mockPhonesSearchRepository).save(testPhones);
    }
    ...
    void deletePhones() throws Exception {
        ...
        // Validate the Phones in Elasticsearch
        verify(mockPhonesSearchRepository, times(1)).deleteById(phones.getId());
    }

    @Test
    @Transactional
    void searchPhones() throws Exception {
        // Configure the mock search repository
        // Initialize the database
        phonesRepository.saveAndFlush(phones);
        when(mockPhonesSearchRepository.search("id:" + phones.getId(), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(phones), PageRequest.of(0, 1), 1));

        // Search the phones
        restPhonesMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + phones.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(phones.getId().intValue())))
            .andExpect(jsonPath("$.[*].phone").value(hasItem(DEFAULT_PHONE)))
