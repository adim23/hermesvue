...
class SocialKindsResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/social-kinds";
     ...
     *
     * @see gr.adr.hermes.repository.search.SocialKindsSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void createSocialKinds() throws Exception {
        ...
        // Validate the SocialKinds in Elasticsearch
        verify(mockSocialKindsSearchRepository, times(1)).save(testSocialKinds);
    }
    ...
    void createSocialKindsWithExistingId() throws Exception {
        ...
        // Validate the SocialKinds in Elasticsearch
        verify(mockSocialKindsSearchRepository, times(0)).save(socialKinds);
    }
    ...
    void putNewSocialKinds() throws Exception {
        ...
        // Validate the SocialKinds in Elasticsearch
        verify(mockSocialKindsSearchRepository).save(testSocialKinds);
    }
    ...
    void deleteSocialKinds() throws Exception {
        ...
        // Validate the SocialKinds in Elasticsearch
        verify(mockSocialKindsSearchRepository, times(1)).deleteById(socialKinds.getId());
    }

    @Test
    @Transactional
    void searchSocialKinds() throws Exception {
        // Configure the mock search repository
        // Initialize the database
        socialKindsRepository.saveAndFlush(socialKinds);
        when(mockSocialKindsSearchRepository.search("id:" + socialKinds.getId(), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(socialKinds), PageRequest.of(0, 1), 1));

        // Search the socialKinds
        restSocialKindsMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + socialKinds.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(socialKinds.getId().intValue())))
            .andExpect(jsonPath("$.[*].code").value(hasItem(DEFAULT_CODE)))
