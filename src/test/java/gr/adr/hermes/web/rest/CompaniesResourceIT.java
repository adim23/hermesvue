...
class CompaniesResourceIT {
     ...
     *
     * @see gr.adr.hermes.repository.search.CompaniesSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void createCompanies() throws Exception {
        ...
        // Validate the Companies in Elasticsearch
        verify(mockCompaniesSearchRepository, times(1)).save(testCompanies);
    }
    ...
    void createCompaniesWithExistingId() throws Exception {
        ...
        // Validate the Companies in Elasticsearch
        verify(mockCompaniesSearchRepository, times(0)).save(companies);
    }
    ...
    void putNewCompanies() throws Exception {
        ...
        // Validate the Companies in Elasticsearch
        verify(mockCompaniesSearchRepository).save(testCompanies);
    }
    ...
    void deleteCompanies() throws Exception {
        ...
        // Validate the Companies in Elasticsearch
        verify(mockCompaniesSearchRepository, times(1)).deleteById(companies.getId());
    }
    ...
    void searchCompanies() throws Exception {
        ...
        // Initialize the database
        companiesRepository.saveAndFlush(companies);
        when(mockCompaniesSearchRepository.search("id:" + companies.getId(), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(companies), PageRequest.of(0, 1), 1));

        // Search the companies
        restCompaniesMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + companies.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(companies.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)));
