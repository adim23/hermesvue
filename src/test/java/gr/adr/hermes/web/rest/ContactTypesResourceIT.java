...
class ContactTypesResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/contact-types";
     ...
     *
     * @see gr.adr.hermes.repository.search.ContactTypesSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void createContactTypes() throws Exception {
        ...
        // Validate the ContactTypes in Elasticsearch
        verify(mockContactTypesSearchRepository, times(1)).save(testContactTypes);
    }
    ...
    void createContactTypesWithExistingId() throws Exception {
        ...
        // Validate the ContactTypes in Elasticsearch
        verify(mockContactTypesSearchRepository, times(0)).save(contactTypes);
    }
    ...
    void putNewContactTypes() throws Exception {
        ...
        // Validate the ContactTypes in Elasticsearch
        verify(mockContactTypesSearchRepository).save(testContactTypes);
    }
    ...
    void deleteContactTypes() throws Exception {
        ...
        // Validate the ContactTypes in Elasticsearch
        verify(mockContactTypesSearchRepository, times(1)).deleteById(contactTypes.getId());
    }

    @Test
    @Transactional
    void searchContactTypes() throws Exception {
        // Configure the mock search repository
        // Initialize the database
        contactTypesRepository.saveAndFlush(contactTypes);
        when(mockContactTypesSearchRepository.search("id:" + contactTypes.getId(), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(contactTypes), PageRequest.of(0, 1), 1));

        // Search the contactTypes
        restContactTypesMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + contactTypes.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(contactTypes.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)));
