...
class SocialContactsResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/social-contacts";
     ...
     *
     * @see gr.adr.hermes.repository.search.SocialContactsSearchRepositoryMockConfiguration
     */
    @Autowired
    private SocialContactsSearchRepository mockSocialContactsSearchRepository;
    ...
    void createSocialContacts() throws Exception {
        ...
        // Validate the SocialContacts in Elasticsearch
        verify(mockSocialContactsSearchRepository, times(1)).save(testSocialContacts);
    }
    ...
    void createSocialContactsWithExistingId() throws Exception {
        ...
        // Validate the SocialContacts in Elasticsearch
        verify(mockSocialContactsSearchRepository, times(0)).save(socialContacts);
    }
    ...
    void putNewSocialContacts() throws Exception {
        ...
        // Validate the SocialContacts in Elasticsearch
        verify(mockSocialContactsSearchRepository).save(testSocialContacts);
    }
    ...
    void deleteSocialContacts() throws Exception {
        ...
        // Validate the SocialContacts in Elasticsearch
        verify(mockSocialContactsSearchRepository, times(1)).deleteById(socialContacts.getId());
    }

    @Test
    @Transactional
    void searchSocialContacts() throws Exception {
        // Configure the mock search repository
        // Initialize the database
        socialContactsRepository.saveAndFlush(socialContacts);
        when(mockSocialContactsSearchRepository.search("id:" + socialContacts.getId(), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(socialContacts), PageRequest.of(0, 1), 1));

        // Search the socialContacts
        restSocialContactsMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + socialContacts.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(socialContacts.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)))
