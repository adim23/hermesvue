...
class EmailsResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/emails";
     ...
     *
     * @see gr.adr.hermes.repository.search.EmailsSearchRepositoryMockConfiguration
     */
    @Autowired
    private EmailsSearchRepository mockEmailsSearchRepository;
    ...
    void createEmails() throws Exception {
        ...
        // Validate the Emails in Elasticsearch
        verify(mockEmailsSearchRepository, times(1)).save(testEmails);
    }
    ...
    void createEmailsWithExistingId() throws Exception {
        ...
        // Validate the Emails in Elasticsearch
        verify(mockEmailsSearchRepository, times(0)).save(emails);
    }
    ...
    void putNewEmails() throws Exception {
        ...
        // Validate the Emails in Elasticsearch
        verify(mockEmailsSearchRepository).save(testEmails);
    }
    ...
    void deleteEmails() throws Exception {
        ...
        // Validate the Emails in Elasticsearch
        verify(mockEmailsSearchRepository, times(1)).deleteById(emails.getId());
    }
    ...
    void searchEmails() throws Exception {
        ...
        // Initialize the database
        emailsRepository.saveAndFlush(emails);
        when(mockEmailsSearchRepository.search("id:" + emails.getId(), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(emails), PageRequest.of(0, 1), 1));

        // Search the emails
        restEmailsMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + emails.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(emails.getId().intValue())))
            .andExpect(jsonPath("$.[*].email").value(hasItem(DEFAULT_EMAIL)))
