...
class CitiesResourceIT {
     ...
     *
     * @see gr.adr.hermes.repository.search.CitiesSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void putNewCities() throws Exception {
        ...
        // Validate the Cities in Elasticsearch
        verify(mockCitiesSearchRepository).save(testCities);
    }
    ...
    void searchCities() throws Exception {
        ...
        // Initialize the database
        citiesRepository.saveAndFlush(cities);
        when(mockCitiesSearchRepository.search("id:" + cities.getId(), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(cities), PageRequest.of(0, 1), 1));

        // Search the cities
        restCitiesMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + cities.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(cities.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)))
