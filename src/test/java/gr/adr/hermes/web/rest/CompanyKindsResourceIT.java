...
class CompanyKindsResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/company-kinds";
     ...
     *
     * @see gr.adr.hermes.repository.search.CompanyKindsSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void createCompanyKinds() throws Exception {
        ...
        // Validate the CompanyKinds in Elasticsearch
        verify(mockCompanyKindsSearchRepository, times(1)).save(testCompanyKinds);
    }
    ...
    void createCompanyKindsWithExistingId() throws Exception {
        ...
        // Validate the CompanyKinds in Elasticsearch
        verify(mockCompanyKindsSearchRepository, times(0)).save(companyKinds);
    }
    ...
    void putNewCompanyKinds() throws Exception {
        ...
        // Validate the CompanyKinds in Elasticsearch
        verify(mockCompanyKindsSearchRepository).save(testCompanyKinds);
    }
    ...
    void deleteCompanyKinds() throws Exception {
        ...
        // Validate the CompanyKinds in Elasticsearch
        verify(mockCompanyKindsSearchRepository, times(1)).deleteById(companyKinds.getId());
    }

    @Test
    @Transactional
    void searchCompanyKinds() throws Exception {
        // Configure the mock search repository
        // Initialize the database
        companyKindsRepository.saveAndFlush(companyKinds);
        when(mockCompanyKindsSearchRepository.search("id:" + companyKinds.getId(), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(companyKinds), PageRequest.of(0, 1), 1));

        // Search the companyKinds
        restCompanyKindsMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + companyKinds.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(companyKinds.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)));
