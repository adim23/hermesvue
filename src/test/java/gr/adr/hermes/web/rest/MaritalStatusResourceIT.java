...
class MaritalStatusResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/marital-statuses";
     ...
     *
     * @see gr.adr.hermes.repository.search.MaritalStatusSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void createMaritalStatus() throws Exception {
        ...
        verify(mockMaritalStatusSearchRepository, times(1)).save(testMaritalStatus);
    }
    ...
    void createMaritalStatusWithExistingId() throws Exception {
        ...
        // Validate the MaritalStatus in Elasticsearch
        verify(mockMaritalStatusSearchRepository, times(0)).save(maritalStatus);
    }
    ...
    void putNewMaritalStatus() throws Exception {
        ...
        // Validate the MaritalStatus in Elasticsearch
        verify(mockMaritalStatusSearchRepository).save(testMaritalStatus);
    }
    ...
    void deleteMaritalStatus() throws Exception {
        ...
        // Validate the MaritalStatus in Elasticsearch
        verify(mockMaritalStatusSearchRepository, times(1)).deleteById(maritalStatus.getId());
    }

    @Test
    @Transactional
    void searchMaritalStatus() throws Exception {
        // Configure the mock search repository
        // Initialize the database
        maritalStatusRepository.saveAndFlush(maritalStatus);
        when(mockMaritalStatusSearchRepository.search("id:" + maritalStatus.getId(), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(maritalStatus), PageRequest.of(0, 1), 1));

        // Search the maritalStatus
        restMaritalStatusMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + maritalStatus.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(maritalStatus.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)));
