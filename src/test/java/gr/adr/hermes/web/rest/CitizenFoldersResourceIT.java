...
class CitizenFoldersResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/citizen-folders";
     ...
     *
     * @see gr.adr.hermes.repository.search.CitizenFoldersSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void createCitizenFolders() throws Exception {
        ...
        // Validate the CitizenFolders in Elasticsearch
        verify(mockCitizenFoldersSearchRepository, times(1)).save(testCitizenFolders);
    }
    ...
    void createCitizenFoldersWithExistingId() throws Exception {
        ...
        // Validate the CitizenFolders in Elasticsearch
        verify(mockCitizenFoldersSearchRepository, times(0)).save(citizenFolders);
    }
    ...
    void putNewCitizenFolders() throws Exception {
        ...
        // Validate the CitizenFolders in Elasticsearch
        verify(mockCitizenFoldersSearchRepository).save(testCitizenFolders);
    }
    ...
    void deleteCitizenFolders() throws Exception {
        ...
        // Validate the CitizenFolders in Elasticsearch
        verify(mockCitizenFoldersSearchRepository, times(1)).deleteById(citizenFolders.getId());
    }

    @Test
    @Transactional
    void searchCitizenFolders() throws Exception {
        // Configure the mock search repository
        // Initialize the database
        citizenFoldersRepository.saveAndFlush(citizenFolders);
        when(mockCitizenFoldersSearchRepository.search("id:" + citizenFolders.getId(), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(citizenFolders), PageRequest.of(0, 1), 1));

        // Search the citizenFolders
        restCitizenFoldersMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + citizenFolders.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(citizenFolders.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)))
