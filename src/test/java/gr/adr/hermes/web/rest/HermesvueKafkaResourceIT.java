...
class HermesvueKafkaResourceIT {
    ...
    void setup() {
        ...
        HermesvueKafkaResource kafkaResource = new HermesvueKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/hermesvue-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/hermesvue-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
