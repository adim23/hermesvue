...
class ZipCodesResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/zip-codes";
     ...
     *
     * @see gr.adr.hermes.repository.search.ZipCodesSearchRepositoryMockConfiguration
     */
    @Autowired
    private ZipCodesSearchRepository mockZipCodesSearchRepository;
    ...
    void createZipCodes() throws Exception {
        ...
        // Validate the ZipCodes in Elasticsearch
        verify(mockZipCodesSearchRepository, times(1)).save(testZipCodes);
    }
    ...
    void createZipCodesWithExistingId() throws Exception {
        ...
        // Validate the ZipCodes in Elasticsearch
        verify(mockZipCodesSearchRepository, times(0)).save(zipCodes);
    }
    ...
    void putNewZipCodes() throws Exception {
        ...
        // Validate the ZipCodes in Elasticsearch
        verify(mockZipCodesSearchRepository).save(testZipCodes);
    }
    ...
    void deleteZipCodes() throws Exception {
        ...
        // Validate the ZipCodes in Elasticsearch
        verify(mockZipCodesSearchRepository, times(1)).deleteById(zipCodes.getId());
    }

    @Test
    @Transactional
    void searchZipCodes() throws Exception {
        // Configure the mock search repository
        // Initialize the database
        zipCodesRepository.saveAndFlush(zipCodes);
        when(mockZipCodesSearchRepository.search("id:" + zipCodes.getId(), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(zipCodes), PageRequest.of(0, 1), 1));

        // Search the zipCodes
        restZipCodesMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + zipCodes.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(zipCodes.getId().intValue())))
            .andExpect(jsonPath("$.[*].street").value(hasItem(DEFAULT_STREET)))
