...
class CitizensMeetingsResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/citizens-meetings";
     ...
     *
     * @see gr.adr.hermes.repository.search.CitizensMeetingsSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void createCitizensMeetings() throws Exception {
        ...
        // Validate the CitizensMeetings in Elasticsearch
        verify(mockCitizensMeetingsSearchRepository, times(1)).save(testCitizensMeetings);
    }
    ...
    void createCitizensMeetingsWithExistingId() throws Exception {
        ...
        // Validate the CitizensMeetings in Elasticsearch
        verify(mockCitizensMeetingsSearchRepository, times(0)).save(citizensMeetings);
    }
    ...
    void putNewCitizensMeetings() throws Exception {
        ...
        // Validate the CitizensMeetings in Elasticsearch
        verify(mockCitizensMeetingsSearchRepository).save(testCitizensMeetings);
    }
    ...
    void deleteCitizensMeetings() throws Exception {
        ...
        // Validate the CitizensMeetings in Elasticsearch
        verify(mockCitizensMeetingsSearchRepository, times(1)).deleteById(citizensMeetings.getId());
    }

    @Test
    @Transactional
    void searchCitizensMeetings() throws Exception {
        // Configure the mock search repository
        // Initialize the database
        citizensMeetingsRepository.saveAndFlush(citizensMeetings);
        when(mockCitizensMeetingsSearchRepository.search("id:" + citizensMeetings.getId(), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(citizensMeetings), PageRequest.of(0, 1), 1));

        // Search the citizensMeetings
        restCitizensMeetingsMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + citizensMeetings.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(citizensMeetings.getId().intValue())))
            .andExpect(jsonPath("$.[*].meetDate").value(hasItem(DEFAULT_MEET_DATE.toString())))
