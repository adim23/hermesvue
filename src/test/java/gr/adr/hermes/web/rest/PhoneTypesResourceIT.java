...
class PhoneTypesResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/_search/phone-types";
     ...
     *
     * @see gr.adr.hermes.repository.search.PhoneTypesSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    void createPhoneTypes() throws Exception {
        ...
        // Validate the PhoneTypes in Elasticsearch
        verify(mockPhoneTypesSearchRepository, times(1)).save(testPhoneTypes);
    }
    ...
    void createPhoneTypesWithExistingId() throws Exception {
        ...
        // Validate the PhoneTypes in Elasticsearch
        verify(mockPhoneTypesSearchRepository, times(0)).save(phoneTypes);
    }
    ...
    void putNewPhoneTypes() throws Exception {
        ...
        // Validate the PhoneTypes in Elasticsearch
        verify(mockPhoneTypesSearchRepository).save(testPhoneTypes);
    }
    ...
    void deletePhoneTypes() throws Exception {
        ...
        // Validate the PhoneTypes in Elasticsearch
        verify(mockPhoneTypesSearchRepository, times(1)).deleteById(phoneTypes.getId());
    }

    @Test
    @Transactional
    void searchPhoneTypes() throws Exception {
        // Configure the mock search repository
        // Initialize the database
        phoneTypesRepository.saveAndFlush(phoneTypes);
        when(mockPhoneTypesSearchRepository.search("id:" + phoneTypes.getId(), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(phoneTypes), PageRequest.of(0, 1), 1));

        // Search the phoneTypes
        restPhoneTypesMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + phoneTypes.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(phoneTypes.getId().intValue())))
            .andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)));
