<template>
  <div>
    <h2 id="page-heading" data-cy="CitizensRelationsHeading">
      <span v-text="$t('hermesvueApp.citizensRelations.home.title')" id="citizens-relations-heading">Citizens Relations</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('hermesvueApp.citizensRelations.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'CitizensRelationsCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('hermesvueApp.citizensRelations.home.createLabel')"> Create a new Citizens Relations </span>
          </button>
        </router-link>
      </div>
    </h2>
        ...
        <form name="searchForm" class="form-inline" v-on:submit.prevent="search(currentSearch)">
          <div class="input-group w-100 mt-3">
            <input
              ...
              name="currentSearch"
              id="currentSearch"
              v-bind:placeholder="$t('hermesvueApp.citizensRelations.home.search')"
    ...
    <div class="alert alert-warning" v-if="!isFetching && citizensRelations && citizensRelations.length === 0">
      <span v-text="$t('hermesvueApp.citizensRelations.home.notFound')">No citizensRelations found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('name')">
              <span v-text="$t('hermesvueApp.citizensRelations.name')">Name</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'name'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('citizen.id')">
              <span v-text="$t('hermesvueApp.citizensRelations.citizen')">Citizen</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span
          id="hermesvueApp.citizensRelations.delete.question"
      ...
      <div class="modal-body">
        <p id="jhi-delete-citizensRelations-heading" v-text="$t('hermesvueApp.citizensRelations.delete.question', { id: removeId })">
