<template>
  <div>
    <h2 id="page-heading" data-cy="TeamsHeading">
      <span v-text="$t('hermesvueApp.teams.home.title')" id="teams-heading">Teams</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('hermesvueApp.teams.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'TeamsCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('hermesvueApp.teams.home.createLabel')"> Create a new Teams </span>
          </button>
        </router-link>
      </div>
    </h2>
        ...
        <form name="searchForm" class="form-inline" v-on:submit.prevent="search(currentSearch)">
          <div class="input-group w-100 mt-3">
            <input
              ...
              name="currentSearch"
              id="currentSearch"
              v-bind:placeholder="$t('hermesvueApp.teams.home.search')"
    ...
    <div class="alert alert-warning" v-if="!isFetching && teams && teams.length === 0">
      <span v-text="$t('hermesvueApp.teams.home.notFound')">No teams found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('name')">
              <span v-text="$t('hermesvueApp.teams.name')">Name</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="hermesvueApp.teams.delete.question" data-cy="teamsDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-teams-heading" v-text="$t('hermesvueApp.teams.delete.question', { id: removeId })">
