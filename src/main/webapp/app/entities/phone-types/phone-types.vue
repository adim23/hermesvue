<template>
  <div>
    <h2 id="page-heading" data-cy="PhoneTypesHeading">
      <span v-text="$t('hermesvueApp.phoneTypes.home.title')" id="phone-types-heading">Phone Types</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('hermesvueApp.phoneTypes.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'PhoneTypesCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('hermesvueApp.phoneTypes.home.createLabel')"> Create a new Phone Types </span>
          </button>
        </router-link>
      </div>
    </h2>
        ...
        <form name="searchForm" class="form-inline" v-on:submit.prevent="search(currentSearch)">
          <div class="input-group w-100 mt-3">
            <input
              ...
              name="currentSearch"
              id="currentSearch"
              v-bind:placeholder="$t('hermesvueApp.phoneTypes.home.search')"
    ...
    <div class="alert alert-warning" v-if="!isFetching && phoneTypes && phoneTypes.length === 0">
      <span v-text="$t('hermesvueApp.phoneTypes.home.notFound')">No phoneTypes found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('name')">
              <span v-text="$t('hermesvueApp.phoneTypes.name')">Name</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="hermesvueApp.phoneTypes.delete.question" data-cy="phoneTypesDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-phoneTypes-heading" v-text="$t('hermesvueApp.phoneTypes.delete.question', { id: removeId })">
