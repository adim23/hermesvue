<template>
  <div>
    <h2 id="page-heading" data-cy="PhonesHeading">
      <span v-text="$t('hermesvueApp.phones.home.title')" id="phones-heading">Phones</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('hermesvueApp.phones.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'PhonesCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('hermesvueApp.phones.home.createLabel')"> Create a new Phones </span>
          </button>
        </router-link>
      </div>
    </h2>
        ...
        <form name="searchForm" class="form-inline" v-on:submit.prevent="search(currentSearch)">
          <div class="input-group w-100 mt-3">
            <input
              ...
              name="currentSearch"
              id="currentSearch"
              v-bind:placeholder="$t('hermesvueApp.phones.home.search')"
    ...
    <div class="alert alert-warning" v-if="!isFetching && phones && phones.length === 0">
      <span v-text="$t('hermesvueApp.phones.home.notFound')">No phones found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('phone')">
              <span v-text="$t('hermesvueApp.phones.phone')">Phone</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'phone'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('description')">
              <span v-text="$t('hermesvueApp.phones.description')">Description</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'description'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('favourite')">
              <span v-text="$t('hermesvueApp.phones.favourite')">Favourite</span>
            ...
            <th scope="row" v-on:click="changeOrder('kind.name')">
              <span v-text="$t('hermesvueApp.phones.kind')">Kind</span>
            ...
            <th scope="row" v-on:click="changeOrder('company.name')">
              <span v-text="$t('hermesvueApp.phones.company')">Company</span>
            ...
            <th scope="row" v-on:click="changeOrder('citizen.id')">
              <span v-text="$t('hermesvueApp.phones.citizen')">Citizen</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="hermesvueApp.phones.delete.question" data-cy="phonesDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-phones-heading" v-text="$t('hermesvueApp.phones.delete.question', { id: removeId })">
