<template>
  <div>
    <h2 id="page-heading" data-cy="EmailsHeading">
      <span v-text="$t('hermesvueApp.emails.home.title')" id="emails-heading">Emails</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('hermesvueApp.emails.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'EmailsCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('hermesvueApp.emails.home.createLabel')"> Create a new Emails </span>
          </button>
        </router-link>
      </div>
    </h2>
        ...
        <form name="searchForm" class="form-inline" v-on:submit.prevent="search(currentSearch)">
          <div class="input-group w-100 mt-3">
            <input
              ...
              name="currentSearch"
              id="currentSearch"
              v-bind:placeholder="$t('hermesvueApp.emails.home.search')"
    ...
    <div class="alert alert-warning" v-if="!isFetching && emails && emails.length === 0">
      <span v-text="$t('hermesvueApp.emails.home.notFound')">No emails found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('email')">
              <span v-text="$t('hermesvueApp.emails.email')">Email</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'email'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('description')">
              <span v-text="$t('hermesvueApp.emails.description')">Description</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'description'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('favourite')">
              <span v-text="$t('hermesvueApp.emails.favourite')">Favourite</span>
            ...
            <th scope="row" v-on:click="changeOrder('kind.name')">
              <span v-text="$t('hermesvueApp.emails.kind')">Kind</span>
            ...
            <th scope="row" v-on:click="changeOrder('citizen.id')">
              <span v-text="$t('hermesvueApp.emails.citizen')">Citizen</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="hermesvueApp.emails.delete.question" data-cy="emailsDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-emails-heading" v-text="$t('hermesvueApp.emails.delete.question', { id: removeId })">
