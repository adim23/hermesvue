...
public class ZipCodesResource {
    ...
    /**
     * {@code SEARCH  /_search/zip-codes?query=:query} : search for the zipCodes corresponding
     * to the query.
     *
     * @param query the query of the zipCodes search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/zip-codes")
    public ResponseEntity<List<ZipCodes>> searchZipCodes(
        @RequestParam String query,
        @org.springdoc.api.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of ZipCodes for query {}", query);
        Page<ZipCodes> page = zipCodesService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
