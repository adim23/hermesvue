...
public class CitizensMeetingsResource {
    ...
    /**
     * {@code SEARCH  /_search/citizens-meetings?query=:query} : search for the citizensMeetings corresponding
     * to the query.
     *
     * @param query the query of the citizensMeetings search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/citizens-meetings")
    public ResponseEntity<List<CitizensMeetings>> searchCitizensMeetings(
        @RequestParam String query,
        @org.springdoc.api.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of CitizensMeetings for query {}", query);
        Page<CitizensMeetings> page = citizensMeetingsService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
