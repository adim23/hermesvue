...
public class PhoneTypesResource {
    ...
    /**
     * {@code SEARCH  /_search/phone-types?query=:query} : search for the phoneTypes corresponding
     * to the query.
     *
     * @param query the query of the phoneTypes search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/phone-types")
    public ResponseEntity<List<PhoneTypes>> searchPhoneTypes(
        @RequestParam String query,
        @org.springdoc.api.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of PhoneTypes for query {}", query);
        Page<PhoneTypes> page = phoneTypesService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
