...
public class CitizensResource {
    ...
    /**
     * {@code SEARCH  /_search/citizens?query=:query} : search for the citizens corresponding
     * to the query.
     *
     * @param query the query of the citizens search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/citizens")
    public ResponseEntity<List<Citizens>> searchCitizens(
        @RequestParam String query,
        @org.springdoc.api.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of Citizens for query {}", query);
        Page<Citizens> page = citizensService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
