...
public class SocialContactsResource {
    ...
    /**
     * {@code SEARCH  /_search/social-contacts?query=:query} : search for the socialContacts corresponding
     * to the query.
     *
     * @param query the query of the socialContacts search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/social-contacts")
    public ResponseEntity<List<SocialContacts>> searchSocialContacts(
        @RequestParam String query,
        @org.springdoc.api.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of SocialContacts for query {}", query);
        Page<SocialContacts> page = socialContactsService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
