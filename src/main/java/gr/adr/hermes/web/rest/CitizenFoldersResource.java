...
public class CitizenFoldersResource {
    ...
    /**
     * {@code SEARCH  /_search/citizen-folders?query=:query} : search for the citizenFolders corresponding
     * to the query.
     *
     * @param query the query of the citizenFolders search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/citizen-folders")
    public ResponseEntity<List<CitizenFolders>> searchCitizenFolders(
        @RequestParam String query,
        @org.springdoc.api.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of CitizenFolders for query {}", query);
        Page<CitizenFolders> page = citizenFoldersService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
