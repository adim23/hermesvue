...
public class CompaniesResource {
    ...
    /**
     * {@code SEARCH  /_search/companies?query=:query} : search for the companies corresponding
     * to the query.
     *
     * @param query the query of the companies search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/companies")
    public ResponseEntity<List<Companies>> searchCompanies(
        @RequestParam String query,
        @org.springdoc.api.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of Companies for query {}", query);
        Page<Companies> page = companiesService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
