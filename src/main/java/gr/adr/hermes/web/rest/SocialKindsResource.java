...
public class SocialKindsResource {
    ...
    /**
     * {@code SEARCH  /_search/social-kinds?query=:query} : search for the socialKinds corresponding
     * to the query.
     *
     * @param query the query of the socialKinds search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/social-kinds")
    public ResponseEntity<List<SocialKinds>> searchSocialKinds(
        @RequestParam String query,
        @org.springdoc.api.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of SocialKinds for query {}", query);
        Page<SocialKinds> page = socialKindsService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
