...
public class EmailsResource {
    ...
    /**
     * {@code SEARCH  /_search/emails?query=:query} : search for the emails corresponding
     * to the query.
     *
     * @param query the query of the emails search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/emails")
    public ResponseEntity<List<Emails>> searchEmails(
        @RequestParam String query,
        @org.springdoc.api.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of Emails for query {}", query);
        Page<Emails> page = emailsService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
