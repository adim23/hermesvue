...
public class CitizensRelationsResource {
    ...
    /**
     * {@code SEARCH  /_search/citizens-relations?query=:query} : search for the citizensRelations corresponding
     * to the query.
     *
     * @param query the query of the citizensRelations search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/citizens-relations")
    public ResponseEntity<List<CitizensRelations>> searchCitizensRelations(
        @RequestParam String query,
        @org.springdoc.api.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of CitizensRelations for query {}", query);
        Page<CitizensRelations> page = citizensRelationsService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
