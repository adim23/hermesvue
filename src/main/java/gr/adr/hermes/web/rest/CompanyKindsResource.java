...
public class CompanyKindsResource {
    ...
    /**
     * {@code SEARCH  /_search/company-kinds?query=:query} : search for the companyKinds corresponding
     * to the query.
     *
     * @param query the query of the companyKinds search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/company-kinds")
    public ResponseEntity<List<CompanyKinds>> searchCompanyKinds(
        @RequestParam String query,
        @org.springdoc.api.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of CompanyKinds for query {}", query);
        Page<CompanyKinds> page = companyKindsService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
