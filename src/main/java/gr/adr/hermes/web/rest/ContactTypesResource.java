...
public class ContactTypesResource {
    ...
    /**
     * {@code SEARCH  /_search/contact-types?query=:query} : search for the contactTypes corresponding
     * to the query.
     *
     * @param query the query of the contactTypes search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/contact-types")
    public ResponseEntity<List<ContactTypes>> searchContactTypes(
        @RequestParam String query,
        @org.springdoc.api.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of ContactTypes for query {}", query);
        Page<ContactTypes> page = contactTypesService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
