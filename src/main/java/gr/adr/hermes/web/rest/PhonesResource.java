...
public class PhonesResource {
    ...
    /**
     * {@code SEARCH  /_search/phones?query=:query} : search for the phones corresponding
     * to the query.
     *
     * @param query the query of the phones search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/phones")
    public ResponseEntity<List<Phones>> searchPhones(
        @RequestParam String query,
        @org.springdoc.api.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of Phones for query {}", query);
        Page<Phones> page = phonesService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
